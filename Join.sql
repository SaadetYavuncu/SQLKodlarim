...
from workers
group by state
having count(state)>1;--> having group by ardından filtreleme için kullanilir
...
from workers; --distinct clause çağrılan terimlerden tekrarlı olanlarin sadece brincisin alir
...
intersect -->intersect operator: iki sorgu (query) sonucunun(common) degerlerini verir.
...
JOINS : 1)INNER JOIN: Ortak (Common) datayı verir.
		...
		5)SELF JOIN : Tek table üzerinde çalışırken iki table varmış gibi çalışıl*/
...
-- 2) LEFT JOIN
--my_companies table'ındaki companyler için company_name, order_id 
